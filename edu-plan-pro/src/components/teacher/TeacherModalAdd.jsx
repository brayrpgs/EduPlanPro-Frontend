...
async function checkIfIdCardExists(idcard) {
  try {
    const response = await fetch(`http://localhost:3001/teacher?cedula=${idcard}`, {
      method: "GET",
      credentials: "include",
    });
    ...
    const jsonResponse = await response.json();
    // Devuelve `true` si se encuentra un profesor con esa cédula
...
const TeacherModalAdd = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (handleValidate()) {
      ...
      const idCardExists = await checkIfIdCardExists(data.idcard);
      if (idCardExists) {
        SweetAlertError("La cédula ya está registrada.");
        return;
      }
      // Si la cédula no existe, procede con la creación
      fetchTeacherCreate(data);
    } else {
      SweetAlertError("Campos Vacios"); // Muestra el error si falla la validación
    }
  };
